...
The Rincoin Core daemon can be configured to act as a trusted "border
router", implementing the rincoin wire protocol and relay, making
...
The ZeroMQ feature in Rincoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building rincoind:
...
For instance:
    ...
    $ rincoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/rincoind.tx.raw \
...
These options can also be provided in rincoin.conf.
...
From the perspective of rincoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into rincoind directly. Furthermore, no information is
...
using. Rincoind appends an up-counting sequence number to each
