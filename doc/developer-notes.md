...
- [Development guidelines](#development-guidelines)
    - [General Rincoin Core](#general-rincoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play rincoins" on the test network, if you
...
be sent upstream, but bugfixes may also be prudent to PR against Rincoin Core so that they can be integrated
...
file descriptors, it will cause problems with Rincoin's `select()` loop, because
...
compatibility between nodes. This happened in Rincoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Rincoin's
